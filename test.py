...
class TestSP(unittest.TestCase):
	...
	def setUp(self):
		self.SP = SP3(30)
		self.SP.read_file(root + 'nvtest.json', 'json')
	...
	def test_merge(self):
		self.SP.merge_column(['Module','Synopsis', 'Description'],'Train')
		self.assertEqual('Train' in self.SP.data, True)

	def test_split(self):
		result = self.SP.split_by_column('Module', True)
	...
	def test_process_text(self):
		self.SP.merge_column(['Module','Synopsis', 'Description'],'Train')
		text = self.SP.process_text('Train', True, True, True)
	...
	def test_processline(self):
		...
		result1 = self.SP.process_line(sampleline1, True, True)
		result2 = self.SP.process_line(sampleline2, True, True)
